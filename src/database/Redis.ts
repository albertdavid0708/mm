...
function createRedisClient(label = "default") {
    ...
    newClient
        ...
        .on("+node", (data) => console.log(`Redis[${label}]: A new node is connected: `))
        .on("-node", (data) => console.log(`Redis[${label}]: A node is disconnected: `))
        .on("node error", (data) => console.log(`Redis[${label}]: An error occurs when connecting to a node: `))
